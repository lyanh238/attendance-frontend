...
export default function FaceRecognition() {
  ...
  useEffect(() => {
    socketRef.current = new WebSocket('wss://67cc-118-69-233-167.ngrok-free.app/ws/face-recognition');
      ...
      if (data.success) {
        ...
        setAttendanceList(prevList => {
          ...
          croppedImages.forEach(student => {
            ...
            if (!newList.some(s => s.name === studentWithImage.name)) {
  ...
  const cutImagesFromFrame = async (imageBlob, results) => {
    ...
    for (const result of results) {
      ...
      // Tính toán kích thước của ảnh cắt
      const x = bbox[0];
      const y = bbox[1];
  ...
  const startFrameCapture = useCallback(() => {
    intervalRef.current = setInterval(async () => {
      ...
      if (blob) {
        ...
        if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN && pendingRequestsRef.current < 5) {
              ...
              <div className="space-y-3">
                {attendanceList.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-gray-500">No students detected</p>
                  </div>
                ) : (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              Confirmed At: {student.confirmationTime} {/* Thời gian xác nhận */}
                            </p>
                          </div>
                        </div>
